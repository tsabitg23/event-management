...
describe('Event E2E', () => {
    ...
    const moduleRef = await Test.createTestingModule({
      imports: [
        TypeOrmModule.forRootAsync({
            useFactory: () => {
                return {
                    type: process.env.DATABASE_TEST_CLIENT as ("sqlite"),
  ...
  it('/POST event', async () => {
    ...
    expect(response.body.data).toEqual(expect.objectContaining(sampleEventData));
    expect(response.body.data.city).toEqual(expect.objectContaining(sampleCityData));
  ...
  it('should retrieve data via /GET and /GET/:id', async () => {
    ...
    const expectedEventResult = eventData.map(event => expect.objectContaining({
        id: event.id,
        name: event.name,
        price: event.price,
    ...
    expect(getAllEvent.body.data).toEqual(expect.arrayContaining(expectedEventResult));
    ...
    expect(getEventById.body.data).toEqual(expect.objectContaining(expectedSelectedEvent));
